...
def eval_loss(model, loss_function, valid_reader, device):
    ...
    for step, batch in tqdm(enumerate(valid_reader)):
        with torch.no_grad():
            ...
            candidate_embeddings = model(batch['pos_image_inputs'].cuda(),batch['pos_report_inputs'],device)
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % eval_step == 0 and global_step > 0:
                    ...
                    if best_acc <= dev_acc:
                        best_acc = dev_acc
                        saved_path = args.out_path+f"_{epoch}"
...
if __name__ == '__main__':
    ...
    train_dataset = MedDataset(image_processor, tokenizer, train_data,train_pos_data,img_patch_token_size)
    valid_dataset = MedDataset(image_processor, tokenizer, train_data,valid_pos_data,img_patch_token_size,valid_data)
